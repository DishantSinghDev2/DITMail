                  ...
                  <div className="flex justify-between items-start">
                    <div className="text-sm truncate pr-2">
                      <span className={!message.read ? "text-gray-900" : "text-gray-600"}>
                        {folder === "sent" ? `To: ${message.to.join(", ")}` : message.from} {message.messageCount > 1 && <span className={`${!message.read ? "text-gray-600" : "text-gray-500"} ml-1 text-xs`}>{message.messageCount} </span>}
                      </span>
                    </div>
